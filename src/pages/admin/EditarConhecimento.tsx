...
export function EditarConhecimento() {
  ...
  // Pre-fill form once course data is available (wait for Supabase to finish loading)
  useEffect(() => {
    if (coursesLoading || !id) return
    ...
    setStatus((course.status ?? 'published') as 'published' | 'draft')
    setDepartmentIds(course.departmentIds ?? [])
    setLabel(course.label ?? DEFAULT_LABEL)
    if (course.steps?.length) {
      setSteps(course.steps.map(s => ({ id: s.id, title: s.title, content: s.content ?? '' })))
    }
  }, [coursesLoading, courses, id])
  ...
  function saveStep(stepId: string, title: string, content: string) {
    setSteps(prev => prev.map(s => s.id === stepId ? { ...s, title, content } : s))
  ...
  async function handleSubmit(e: { preventDefault(): void }) {
    ...
    const { error: updateErr } = await supabase
      .from('courses')
      .update({ title, description, category, duration, thumbnail, status, label })
      .eq('id', id)
    ...
    // Replace departments
    await supabase.from('course_departments').delete().eq('course_id', id)
    if (departmentIds.length > 0) {
      await supabase.from('course_departments').insert(
        departmentIds.map(department_id => ({ course_id: id, department_id }))
      )
    }
    ...
    setLoading(false)
    setSaved(true)
    setTimeout(() => navigate('/admin/base-conhecimento'), 2000)
  }
  ...
  if (notFound) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center">
        ...
        <p className="font-['Space_Grotesk'] font-semibold text-[var(--color-on-surface)]">Conteúdo não encontrado</p>
        <button onClick={() => navigate('/admin/base-conhecimento')}
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="font-['Space_Grotesk'] text-2xl font-bold text-[var(--color-on-surface)]">Editar Conhecimento</h1>
          <p className="text-sm text-[var(--color-on-surface-variant)] mt-0.5">Atualize as informações e etapas do conteúdo</p>
        </div>
      </div>
      ...
      {saved && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-green-700/20 border border-green-700/30 text-green-400 text-sm">
          ...
          Conhecimento atualizado com sucesso! Redirecionando para a Base de Conhecimento...
        </div>
      )}
      {error && (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
          ...
          <div className="xl:col-span-7 space-y-6">
            ...
            <div className="bg-[var(--color-surface-container)] rounded-xl p-6 space-y-5">
              ...
              <CField label="Descrição">
                <textarea value={description} onChange={e => setDescription(e.target.value)}
                  placeholder="Descreva os objetivos e o público-alvo..."
            ...
            <div className="bg-[var(--color-surface-container)] rounded-xl p-6 space-y-4">
              ...
              {thumbnail && (
                <img src={thumbnail} alt="preview" className="w-full aspect-video object-cover rounded-xl" />
              )}
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button type="submit" disabled={loading || !title.trim()} onClick={() => setStatus('draft')}
              ...
              <button type="submit" disabled={loading || !title.trim()} onClick={() => setStatus('published')}
                ...
                {loading && status === 'published' ? 'Salvando...' : 'Publicar'}
              </button>
            </div>
          </div>
              ...
              <div className="bg-[var(--color-surface-container)] rounded-xl overflow-hidden shadow-[0_8px_30px_color-mix(in_srgb,var(--color-primary)_10%,transparent)]">
                ...
                <div className="p-4 space-y-3">
                  <h3 className="font-['Space_Grotesk'] font-semibold text-[var(--color-on-surface)] text-sm leading-snug line-clamp-2">
                    {title || <span className="text-[var(--color-on-surface-variant)]">Título do conteúdo...</span>}
                  </h3>
                  {duration && (
