...
export function NovaTrilha() {
  ...
  async function handleSubmit(e: { preventDefault(): void }) {
    ...
    const trackId = `trilha-${Date.now()}`
    const { error: trackErr } = await supabase
      .from('tracks')
      .insert({ id: trackId, title, goal, badge_icon: badgeIcon, visibility })

    if (trackErr) {
      setError(trackErr.message)
      setLoading(false)
      return
    }
    ...
    if (departmentIds.length > 0) {
      await supabase.from('track_departments').insert(
        departmentIds.map(department_id => ({ track_id: trackId, department_id }))
      )
    }

    setLoading(false)
    setSaved(true)
    setTimeout(() => navigate('/admin/trilhas'), 2000)
  }
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="font-['Space_Grotesk'] text-2xl font-bold text-[var(--color-on-surface)]">Nova Trilha</h1>
          <p className="text-sm text-[var(--color-on-surface-variant)] mt-0.5">Configure a trilha e ordene os cursos da sequência</p>
        </div>
      </div>
      ...
      {saved && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-green-700/20 border border-green-700/30 text-green-400 text-sm">
          ...
          Trilha criada com sucesso! Redirecionando...
        </div>
      )}
      {error && (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
          ...
          <div className="xl:col-span-5 space-y-6">
            ...
            <div className="bg-[var(--color-surface-container)] rounded-xl p-5">
              <p className="text-xs uppercase tracking-widest text-[var(--color-on-surface-variant)] mb-4">Pré-visualização</p>
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-xl bg-[var(--color-surface-highest)] flex items-center justify-center flex-shrink-0">
                  <span className="material-symbols-rounded text-[var(--color-primary)] text-2xl">{badgeIcon}</span>
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="font-['Space_Grotesk'] font-semibold text-[var(--color-on-surface)] leading-snug">
                    {title || <span className="text-[var(--color-on-surface-variant)]">Nome da trilha...</span>}
                  </h3>
                  {goal && <p className="text-xs text-[var(--color-on-surface-variant)] mt-1 line-clamp-2">{goal}</p>}
                </div>
              </div>
              <div className="mt-4 text-xs text-[var(--color-on-surface-variant)] flex items-center gap-1">
                <span className="material-symbols-rounded text-sm">play_circle</span>
                {selectedIds.length} curso{selectedIds.length !== 1 ? 's' : ''} selecionado{selectedIds.length !== 1 ? 's' : ''}
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button type="submit" disabled={!title.trim() || selectedIds.length === 0 || loading}
